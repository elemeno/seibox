...
def save_label(labels_path: str, record_id: str, label: str, rationale: str, 
               model_output: str, current_judgement: Dict[str, Any]):
...
def main():
    ...
    st.set_page_config(
        page_title="Safety Eval Labeler",
        page_icon="🏷️",
        layout="wide"
    )
    ...
    suite_filter = st.sidebar.selectbox(
        "Filter by suite",
        ["All"] + list(set(r.id.split("_")[0] for r in records)),
        index=0
    )
    ...
    with st.form("labeling_form"):
        ...
        with col1:
            label = st.selectbox(
                ...
                index=["Correct", "Incorrect", "Unsure"].index(existing_label['label']) 
                      if existing_label else 0
            )
    ...
    with nav_col2:
        ...
        unlabeled_indices = [i for i, r in enumerate(filtered_records) 
