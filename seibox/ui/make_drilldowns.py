...
from seibox.ui.drilldown import render_drilldown, create_index
...
def parse_run_filename(filename: str) -> Tuple[str, str, str]:
    ...
    if category_idx > 0 and len(parts) > category_idx:
        ...
        profile = "_".join(parts[category_idx + 1:]) if category_idx + 1 < len(parts) else "baseline"
    ...
    with open(run_file, "r") as f:
        for line_num, line in enumerate(f):
            ...
            try:
                ...
                console.print(f"[yellow]Warning: Failed to parse line {line_num} in {run_file}: {e}[/yellow]")
...
@click.option(
    "--out",
    required=True,
    help="Output directory for drilldown HTML files"
)
@click.option(
    "--max-per-run",
    default=None,
