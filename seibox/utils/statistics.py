        ...
        for _, row in df_copy.iterrows():
            ...
            if severity is None:
                if row["id"].startswith("pii"):
                    severity = "high" if "credit" in row.get("text", "") or "email" in row.get("text", "") else "med"
...
def find_flips_with_details(
    ...
    for _, row in merged.iterrows():
        if row["id"].startswith("pii"):
            severity = "high" if "credit" in row["text_baseline"] or "email" in row["text_baseline"] else "med"
        elif row["id"].startswith("injection"):
...
def format_confidence_interval(value: float, ci: Tuple[float, float], percentage: bool = True) -> str:
