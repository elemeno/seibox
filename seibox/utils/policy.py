...
class PolicyManager:
    ...
    def get_refusal_message(self, policy_name: str, context: str = "general") -> str:
        ...
        try:
            ...
            # Get base template
            template = policy.get("refusal_templates", {}).get("standard", 
                "Sorry—I can't disclose personal data. I can {alternative} instead.")
            
            # Get context-specific alternative
            alternatives = policy.get("refusal_templates", {}).get("alternatives_by_context", {})
            alternative = alternatives.get(context, alternatives.get("general", 
                "help with general guidance or templates"))
...
def get_contextual_refusal(content_type: str = "general", detected_pii: Optional[Dict] = None) -> str:
