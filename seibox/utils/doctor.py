...
def check_cache_directory() -> Dict[str, any]:
    ...
    if cache_status["writable"]:
        ...
        console.print(f"  ✓ Size: {cache_status['size_mb']:.1f} MB ({cache_status['files_count']} files)")
    else:
...
def estimate_run_cost(config_path: str, suite_name: Optional[str] = None) -> Dict[str, any]:
    ...
    try:
        ...
        estimation = {
            ...
            "cost_per_1k": estimated_cost.get("usd", 0.0) * 1000 / total_records if total_records > 0 else 0
        }
...
def run_health_checks(config_path: Optional[str] = None, model_name: Optional[str] = None, suite_name: Optional[str] = None):
