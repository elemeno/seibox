...
def _import_csv_labels(labels_file: Path, out_path: str) -> Dict[str, Any]:
    ...
    with open(labels_file, "r", encoding="utf-8") as csvfile:
        ...
        for col in available_cols:
            col_lower = col.lower()
            if not label_col and any(
                term in col_lower for term in ["label", "blocked", "block", "decision", "verdict"]
            ):
                label_col = col
            elif not reviewer_col and any(
                term in col_lower for term in ["reviewer", "labeler", "annotator", "human"]
            ):
                reviewer_col = col
            elif not notes_col and any(
                term in col_lower for term in ["note", "comment", "rationale", "reason"]
            ):
                notes_col = col
        ...
        if not label_col:
            ...
            if not label_col:
                raise ValueError(
                    f"No label column found. Expected one of: human_label, label, blocked, decision. Available: {available_cols}"
                )
