...
class MatrixOrchestrator:
    ...
    def load_categories(self, categories_path: str = "configs/categories.yaml") -> List[Dict[str, Any]]:
    ...
    def estimate_job_metrics(self, model: Dict[str, Any], category: Dict[str, Any], 
                           sample_size: int) -> Tuple[int, float, int]:
    ...
    def plan(self, models: Optional[List[str]] = None, categories: Optional[List[str]] = None,
             sample_mode: str = "SMOKE", outdir: str = "runs/landscape") -> Plan:
                ...
                for future in as_completed(future_to_job):
                    ...
                    try:
                        ...
                        for i, plan_job in enumerate(plan.jobs):
                            if (plan_job.model == completed_job.model and 
                                plan_job.category == completed_job.category):
                        ...
                        if completed_job.status == "completed":
                            console.print(f"[green]✓[/green] {completed_job.model} x {completed_job.category}")
                        else:
                            console.print(f"[red]✗[/red] {completed_job.model} x {completed_job.category}: {completed_job.error}")
