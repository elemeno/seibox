...
MITIGATION_COMBOS = {
    "baseline": {"name": "Baseline", "description": "No mitigations applied", "mitigation_ids": []},
    "policy_gate": {
...
class ReleaseOrchestrator:
    ...
    def get_safety_categories(self) -> List[Dict]:
        ...
        if len(safety_categories) != 3:
            console.print(
                f"[yellow]Warning: Expected 3 safety categories, found {len(safety_categories)}[/yellow]"
            )
    ...
    def create_release_plan(
        ...
        console.print(
            f"[bold blue]Creating release plan for {len(enabled_models)} models across {len(safety_categories)} categories with {len(MITIGATION_COMBOS)} mitigation configs[/bold blue]"
        )
    ...
    def execute_release(self, plan: Plan, resume: bool = True, max_workers: int = 2) -> Plan:
...
def run_release_evaluation(
    ...
    plan = orchestrator.create_release_plan(sample_mode=sample_mode, outdir=outdir, models=models)
    ...
    completed_plan = orchestrator.execute_release(plan, resume=True, max_workers=max_workers)
